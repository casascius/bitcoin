...
static unsigned long f(unsigned long B,unsigned long C,unsigned long D, int t)
{
	if (t < 20)
	{
		return ((B & C)^((~B) & D));
...
static unsigned long Rol(unsigned long x, int y)
{
	if (y % 32 == 0) {return x;} 
	else {return ((x << y)^(x >> -y));}
}

static unsigned long Ror(unsigned long x, int y)
{
	if (y % 32 == 0) {return x;}
	else {return ((x >> y)^(x << -y));}
}

static void append(char* s, int slen, char c)
{
	//int len = strlen(s);
	s[slen] = c;
	s[slen+1] = '\0';
...
static void SHA1(const char s[],int slen, unsigned long *H)
{
	unsigned long K[80];
...
static void HMAC(const char text[], int tlen, const char key[], int klen, unsigned long *H)
{
	char c;
	char s[256] = "";
	int slen = 0;
